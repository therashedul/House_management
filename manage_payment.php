  ...
  <form action="" id="manage-payment">
    ...
    <div class="form-group">
        ...
        <select name="tenant_id" id="tenant_id" class="custom-select select2">
            ...
            $tenant = $conn->query("SELECT slipes.*, tenants.* FROM slipes  RIGHT JOIN tenants ON tenants.id = slipes.tenant_id WHERE tenants.status = 1  GROUP BY tenants.id  ORDER by tenants.id ASC;");;        
...
<div id="details_clone" style="display: none">
    <div class='d'>
        ...
        <hr>
        <input type="hidden" class="slipId"  name="slip_id">
        <input type="hidden" class="status"  name="activet">    
        ...
        $.ajax({
            ...
            success:function(resp){
                if(resp){
                    ...
                    details.find('.slipId').val(resp.slip_id)      
                    details.find('.status').val(resp.status)                

...
<?php 
$tenants =$conn->query("SELECT t.*,concat(t.fullname) as name,h.total_bill FROM tenants t inner join slipes h on h.tenant_id = t.id where t.id = {$_GET['id']} ");
		...
		<div class="row">
			<div class="col-md-4">
				<div id="details">
					...
					<p>Monthly Rental Rate: <b><?php echo number_format($total_bill,2) ?></b></p>
					<p>Outstanding Balance: <b><?php echo number_format($outstanding,2) ?></b></p>
					<p>Total Paid: <b><?php echo number_format($paid,2) ?></b></p>
					<p>Due Amount: <b><?php echo number_format($outstanding,2) ?></b></p>

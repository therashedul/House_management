				...
				<div class="card-body">					
					<table class="table table-condensed table-bordered table-hover" id="example">
						...
						<tbody>
							<?php 
							$i = 1;
							$slip = $conn->query("SELECT  s.*,  t.fullname, t.house_no FROM slipes s INNER JOIN tenants t on s.tenant_id = t.id  where s.status = 1 ORDER by s.id desc");
							while($row=$slip->fetch_assoc()):
								...
								<tr>
									...
									<td>
										<p><?php echo number_format($row['total_unit']); ?></p>
									</td>
									<td>
										<p><b><?php echo number_format($row['total_bill']); ?></b></p>
									</td>
									<td>									
										<p><?php echo $row['due_bill'] ?></p>
									</td>
									<td>
